...
public class AdjacencyMatrix {
	...
	public AdjacencyMatrix(String[] args)
	{
		...
		this._map = new boolean[this._size][this._size];
	}
	...
	public void add(String v, String w)
	{
		...
		if(i < 0 || j < 0)
			return;
		this._map[i][j] = this._map[j][i] = true;
	}
	...
	private String vertex(int i)
	{
		...
		for(int j = 0; j< this._size; j++)
			if(this._map[i][j]) buf.append(this._vertices[j]+" ");
		return buf+"";
	}
	...
	public String DFS(int i)
	{
		...
		do{
			...
			{
				if((!visit[j]) && this._map[offset][j])  //adjacency and not visited yet
				{
